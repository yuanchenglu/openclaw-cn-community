...
来源: https://clawd.org.cn/providers/moonshot.html

# Moonshot AI (Kimi)

Moonshot 提供具有 OpenAI 兼容端点的 Kimi API。配置提供者并将默认模型设置为 `moonshot/kimi-k2-0905-preview`，或使用 Kimi Code 和 `kimi-code/kimi-for-coding`。
...
注意：Moonshot 和 Kimi Code 是独立的提供者。密钥不可互换，端点不同，模型引用也不同（Moonshot 使用 `moonshot/...`，Kimi Code 使用 `kimi-code/...`）。
